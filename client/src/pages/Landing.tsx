...
const browsers = [
  ...
  { name: "Edge", family: "Chromium", icon: "/manus-storage/edge_a294e85b.svg", download: CHROMIUM_PACKAGE, file: "quickcopy-chromium.zip" },
  ...
  { name: "Safari", family: "Separate target", icon: "/manus-storage/safari_156e30d9.svg" },
];
  ...
  return <div className="marketing-shell">
    ...
    <main>
      ...
      <section id="compatibility" className="compatibility-section compatibility-single">
        <div className="compatibility-copy"><div className="marketing-kicker"><span /> DESKTOP EXTENSION COVERAGE</div><h2>Built for the browsers<br />people actually use.</h2><p>Choose your browser and download its matching package. Firefox receives its own build; Chrome, Brave, Edge, Opera, and Arc share the Chromium build. Safari is listed separately because it needs a dedicated Apple wrapper.</p><div className="compatibility-status"><span>INSTALL CONSOLE</span><strong><i /> 6 TARGETS · 2 PACKAGES</strong><code>QC/EXT-01</code></div><div className="browser-downloads" aria-label="QuickCopy browser package downloads">{browsers.map((browser) => browser.download ? <a key={browser.name} className="browser-download" href={browser.download} download={browser.file} title={`Download QuickCopy for ${browser.name}`}><img src={browser.icon} alt="" /><span><strong>{browser.name}</strong><small>{browser.family}</small></span><ArrowUpRight size={15} /></a> : <div key={browser.name} className="browser-download is-separate" title="Safari wrapper required before download"><img src={browser.icon} alt="" /><span><strong>{browser.name}</strong><small>{browser.family}</small></span><em>WRAPPER</em></div>)}</div><p className="download-note">Both package manifests are built and checked; the Chromium package is smoke-loaded in Chromium in this workspace. Firefox needs signed distribution for persistent installation. Safari needs an Xcode Safari Web Extension wrapper before it can have a real download. The dashboard itself is available for wallet management in Android and iOS browsers.</p></div>
      </section>
